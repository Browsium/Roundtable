...
import { Plus, Edit, Trash2, RefreshCw, User, Upload } from 'lucide-react';
...
export default function PersonasPage() {
  ...
  const handleDeploy = async (id: string) => {
    try {
      setError(null);
      setDeployingId(id);
      await personaApi.deploy(id);
      await loadPersonas();
    } catch (err: any) {
      setError(err?.message || 'Failed to deploy persona');
    } finally {
      setDeployingId(null);
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={deployingId === persona.id}
                className="inline-flex items-center justify-center px-3 py-2 border border-green-300 rounded-md text-sm font-medium text-green-700 bg-white hover:bg-green-50 disabled:opacity-50 disabled:cursor-not-allowed"
                title={persona.status === 'deployed' ? 'Redeploy to CLIBridge' : 'Deploy to CLIBridge'}
              >
                {deployingId === persona.id ? (
